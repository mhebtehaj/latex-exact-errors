...
async function main() {
  ...
  const temporaryRoot = await fs.realpath(await fs.mkdtemp(path.join(process.platform === 'darwin' ? '/private/tmp' : os.tmpdir(), 'lex-live-')));
  ...
  let passed = false;
  try {
    await Promise.all([fs.mkdir(path.join(project, '.vscode'), { recursive: true }),
    ...
    await runTests({
      ...
      extensionTestsEnv: {
        ...
        LATEX_EXACT_HOST_ENGINE: engine, LATEX_LIVE_KEEP: process.env.LATEX_LIVE_KEEP || ''
      }
    });
